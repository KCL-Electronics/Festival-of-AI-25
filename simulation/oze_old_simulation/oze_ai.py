...
def log_ai(player_obj, message_type, message):
    ...
    if player_obj and player_focus_num != 0:
        # If a specific AI player is focused, only log for them OR if the log is not player-specific (player_obj is None)
        if is_ai_player and player_obj.player_num != player_focus_num:
            return
        # Allow non-player specific game logs if focus is on
        elif not is_ai_player and message_type != "GAME_LOG": # Let GAME_LOG pass if player_obj is None/not AI player
             pass
...
class Player(Entity):
    ...
    def condition_TeammateHasBall(self, game_state):
        ...
        if teammates:
            # Check if the first teammate (assuming only one for now) has the ball
            return teammates[0].pos_m.distance_squared_to(ball.pos_m) < CFG["AI_HAS_BALL_THRESHOLD_M"]**2
        return False

    def condition_BallInMyDefensiveThird(self, game_state): # Phase 2: Using ball's position
        ball, _, _, _, _, _ = self._get_game_context_objects(game_state)
    ...
    def _is_path_clear(self, start_pos, end_pos, obstacles, clearance_radius): # Phase 3: Robust perpendicular distance
        ...
        for obs in obstacles:
            ...
            dist_to_line_sq = 0
            if proj_scalar < 0: # Obstacle is behind start point relative to path
                dist_to_line_sq = to_obs_vec.length_squared()
            elif proj_scalar > path_len: # Obstacle is beyond end point relative to path
                dist_to_line_sq = (obs.pos_m - end_pos).length_squared()
            else: # Obstacle projection is within the segment
                # Perpendicular distance squared to the line containing the segment
                # Using (Ax - By)^2 / (A^2 + B^2) where line is Ax + By + C = 0
                # For path_dir = (dx, dy), perpendicular is (-dy, dx)
                # Dist from point (px, py) to line through (x1,y1) with dir (dx,dy) is abs(dy(px-x1) - dx(py-y1)) / sqrt(dx^2+dy^2)
                # Since path_dir is normalized, sqrt(dx^2+dy^2) = 1
                # (to_obs_vec.x * path_dir.y - to_obs_vec.y * path_dir.x)**2 is equivalent to ( (to_obs_vec) X path_dir )^2
                # if path_dir is (dx, dy), then (to_obs_vec - path_dir * proj_scalar) is the perpendicular vector.
                # dist_to_line_sq = (to_obs_vec - path_dir * proj_scalar).length_squared() # Original robust one
                # Simplified if path_dir is normalized: (to_obs_vec.x*path_dir.y - to_obs_vec.y*path_dir.x)**2
                # Let's use the vector subtraction for clarity, assuming safe_normalize worked.
            ...
            if dist_to_line_sq < combined_radius_for_collision**2:
                # An additional check: if the projection is within the segment (or very close to ends)
                # This prevents false positives if an obstacle is near the line but far along its extension
                if -obs.radius_m < proj_scalar < path_len + obs.radius_m: # Check if projection falls within segment
    ...
    def condition_CanShoot(self, game_state): # PHASE 2 MODIFIED
        ...
        player_focus_check = True # Default to true for non-focused or general logging
        if self.is_ai : # Apply focus check only if player is AI
            player_focus_num = CFG.get("DEBUG_AI_PLAYER_FOCUS_NUM", 0)
        ...
        opponent_half_shoot_start_factor_a = CFG.get("AI_SHOOT_MIN_X_OWN_HALF_A", 1.0 - own_half_shoot_limit_factor_b)
        ...
        if self.team == 'B' and self.pos_m.x > CFG["ARENA_WIDTH_M"] * own_half_shoot_limit_factor_b:
            ...
            return False
        # (Team A AI check can be added here if needed for symmetry)
        ...
        if self.vel_m_s.length_squared() > 0.01:
            ...
            if dir_to_goal.length_squared() > 1e-6: # Only check angle if not already at goal
    ...
    def condition_IsMyGoalThreatened(self, game_state):
        ...
        ball_in_def_third = self.condition_BallInMyDefensiveThird(game_state) # Uses ball's position
    ...
    def _is_at_target(self, target_threshold_factor=None):
        if target_threshold_factor is None:
            target_threshold_factor = CFG["AI_ARRIVAL_THRESHOLD_FACTOR"] # Uses updated tighter threshold
    ...
    def _execute_kick_action(self, target_pos, action_name, game_state):
        ...
        # No player movement steering for kick itself, it's an instantaneous event
        # self._set_steering_target_for_action(target_pos, "AI_Slowing_RADIUS_POSITION", action_name)
    ...
    def action_ClearBall(self, game_state): # PHASE 3 MODIFIED
        if not self.condition_CanKickBall(game_state): return "FAILURE"
    ...
    def action_DribbleStrategically(self, game_state): # PHASE 2 MODIFIED
        ...
        action_debug_name_suffix = self.current_action_name_for_debug # Preserve current suffix if not recalculating
        ...
        else: # Not recalculating, just persisting
    ...
    def _move_to_position_action(self, target_pos, action_name, game_state, slowing_radius_key="AI_Slowing_RADIUS_POSITION"): # PHASE 3 TWEAK
    ...
    def action_MoveToDefensiveCover(self, game_state): # PHASE 3 MODIFIED
        ball, _, opponents, my_goal_center, _, _ = self._get_game_context_objects(game_state)
        ...
        if recalculate_target:
            ...
            max_x_a = CFG["ARENA_WIDTH_M"] * (1.0 - CFG.get("AI_DEFENDER_MIN_X_B_FACTOR", 0.45)) # Adjusted for A
            if self.team == 'B':
    ...
    def action_HoldMidfieldLine(self, game_state): # PHASE 3 MODIFIED
        ball, _, _, _, _, _ = self._get_game_context_objects(game_state)
        ...
        target_y = max(CFG["ARENA_HEIGHT_M"] * 0.25, min(target_y, CFG["ARENA_HEIGHT_M"] * 0.75)) # Clamp Y to middle half of pitch
    ...
    def _build_behavior_tree(self): # PHASE 2 MODIFIED for Attacker
        p = self
        if self.player_num == 1:
        ...
        elif self.player_num == 2: # Defender AI (B2)
            self.behavior_tree = Selector(f"ROOT_Defender_{p}", player=p, children=[
                ...
                ActionNode("MoveToDefensiveCover", player=p) # Fallback if other defender actions fail
            ])

    def update_ai(self, dt, ball_obj, all_players_list, is_designated_ball_winner_for_me):
        if not self.is_ai or not self.behavior_tree:
            super().update(dt)
            return
        ...
        if just_gained_ball or just_lost_ball or designation_changed:
            self.dribble_target_m = None # Reset any persisted dribble target
            self.dribble_target_persists_frames = 0
            self.defensive_cover_target_m = None # Reset persisted defensive cover target
            self.defensive_cover_persists_frames = 0
            # Note: last_ball_y_for_midfield_hold is not reset here to prevent jitter if HoldMidfieldLine is re-entered quickly
        ...
        if CFG.get("DEBUG_LOG_AI_STEERING", False) and player_focus_check:
            log_ai(self, "STEERING_FORCES", f"ObjectiveF: {force_objective.length():.2f} (w:{CFG['AI_WEIGHT_PRIMARY_OBJECTIVE']}), SeparationF: {force_separation.length():.2f} (w:{CFG['AI_WEIGHT_SEPARATION']})")
        ...
        if player_focus_check:
            ...
            log_ai(self, "TICK_END", f"NewAction: {self.current_action_name_for_debug}, BT: {bt_status}, Target: {target_str}, Vel: {self.vel_m_s.length():.2f}")
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("2v2 Refined BT AI Football Mk4.2 - Full Phases") # Updated caption
        ...
        print("Game initialized with Refined Behavior Tree AI Mk4.2 (Full Phases).")
        if _ai_log_file_handle:
             _ai_log_file_handle.write("Game class initialized (Full Phases).\n")
             _ai_log_file_handle.flush()
    ...
    def get_initial_player_positions(self):
        ...
        return [
            ...
            (pygame.math.Vector2(CFG["ARENA_WIDTH_M"] - q_w, t_h1), 'B', 1),
            (pygame.math.Vector2(CFG["ARENA_WIDTH_M"] - q_w, t_h2), 'B', 2),
        ]

    def setup_entities_for_kickoff(self): # ADDED RESETS FOR PHASE 2/3 ATTRIBUTES
        self.players.clear()
    ...
    def handle_human_input(self):
        ...
        if self.players: self.players[0].set_movement_input(p1_move_vec)
        ...
        if len(self.players) > 1: self.players[1].set_movement_input(p2_move_vec)
    ...
    def update_game_state(self, dt):
        ...
        if len(ai_team_players_b) >= 1:
            if len(ai_team_players_b) == 1:
                ...
                if not p_b_attacker and p_b_defender: p_b_attacker = p_b_defender
                elif not p_b_defender and p_b_attacker: p_b_defender = p_b_attacker
                elif not p_b_attacker and not p_b_defender:
                ...
                if p_b_attacker and p_b_defender:
        ...
        if scored_this_frame:
            ...
            for p_reset in self.players:
                if p_reset.is_ai and p_reset.behavior_tree:
                    ...
                    # Reset Phase 2/3 attributes on goal
    ...
    def game_loop(self):
        ...
        while is_running:
            ...
            if self.game_active: self.handle_human_input()
            self.update_game_state(dt)
            self.render_all()
        pygame.quit()
        print("Game exited cleanly.")
        if _ai_log_file_handle:
            _ai_log_file_handle.write("Game exited cleanly via Pygame quit.\n")
            _ai_log_file_handle.flush()

if __name__ == '__main__':
    # These temp variables are used to define CFG above, then are no longer needed globally.
    # Their values are now baked into the CFG dictionary.
    ...
    try:
        _ai_log_file_handle = open(_AI_LOG_FILE_PATH, "w", encoding="utf-8") # "w" to overwrite for new test session
        current_time_str = str(pygame.time.get_ticks())
